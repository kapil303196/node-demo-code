...
const loginUser = async (username, password, platform, roleAccess) => {
  try {
    ...
    let user = await dbService.findOne(Admin, where);
    if (user) {
...
const changePassword = async (params) => {
  try {
    ...
    if (user && user.id) {
      ...
      let updatedUser = dbService.updateOne(Admin, where, { password: password });
      if (updatedUser) {
        return {
          flag: false,
...
const sendResetPasswordNotification = async (user) => {
  ...
  try {
    ...
    await dbService.updateOne(Admin, where, {
      resetPasswordLink: {
        code: token,
        expireTime: expires,
      },
...
const resetPassword = async (user, newPassword) => {
  try {
    ...
    const dbUser = await dbService.findOne(Admin, where);
    if (!dbUser) {
      return {
        flag: true,
        data: 'User not found',
